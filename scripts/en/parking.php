...
case when (c.present/c.total) >= 0.6 or current_status = 'Present' then 'red' 
  when (c.present/c.total)  < 0.6 and (c.present/c.total) >= 0.3 then 'blue' 
  when (c.present/c.total)  < 0.3 or current_status = 'Unocccupied' then 'green' end as occRate
from 
tb_park_sensor_track a left join tb_restrict b on a.restrict_id = b.id
inner join tb_occ_rate c on a.rd_seg_id = c.rd_seg_id
where 
